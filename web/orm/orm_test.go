...
func TestLimit(t *testing.T) {
	...
	num, err := qs.Limit(1).All(&posts, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
	...
	num, err = qs.Limit(-1, 2).All(&posts, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 2))

	num, err = qs.Limit(0, 2).All(&posts, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 2))
}
...
func TestOffset(t *testing.T) {
	var posts []*Post
	qs := dORM.QueryTable("post")
	num, err := qs.Limit(1).Offset(2).All(&posts, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))

	num, err = qs.Offset(2).All(&posts, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 2))
}
...
func TestAll(t *testing.T) {
	var users []*User
	qs := dORM.QueryTable("user")
	num, err := qs.OrderBy("Id").All(&users, nil)
	throwFail(t, err)
	...
	var users2 []User
	qs = dORM.QueryTable("user")
	num, err = qs.OrderBy("Id").All(&users2, nil)
	throwFail(t, err)
	throwFailNow(t, AssertIs(num, 3))
	...
	qs = dORM.QueryTable("user")
	num, err = qs.OrderBy("Id").RelatedSel().All(&users2, []string{"UserName"})
	throwFail(t, err)
	throwFailNow(t, AssertIs(num, 3))
	...
	qs = dORM.QueryTable("user")
	num, err = qs.Filter("user_name", "nothing").All(&users, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 0))

	var users3 []*User
	qs = dORM.QueryTable("user")
	num, err = qs.Filter("user_name", "nothing").All(&users3, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 0))
...
func TestOne(t *testing.T) {
	...
	user = User{}
	err = qs.OrderBy("Id").Limit(1).One(&user, nil)
	throwFailNow(t, err)
	throwFail(t, AssertIs(user.UserName, "slene"))
	throwFail(t, AssertNot(err, ErrMultiRows))

	user = User{}
	err = qs.OrderBy("-Id").Limit(100).One(&user, nil)
	throwFailNow(t, err)
	throwFail(t, AssertIs(user.UserName, "nobody"))
	throwFail(t, AssertNot(err, ErrMultiRows))

	err = qs.Filter("user_name", "nothing").One(&user, nil)
	throwFail(t, AssertIs(err, ErrNoRows))
...
func TestRelatedSel(t *testing.T) {
	...
	var user User
	err = qs.Filter("user_name", "slene").RelatedSel("profile").One(&user, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
	...
	err = qs.Filter("user_name", "slene").RelatedSel().One(&user, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
	throwFail(t, AssertNot(user.Profile, nil))
	if user.Profile != nil {
	...
	err = qs.Filter("user_name", "nobody").RelatedSel("profile").One(&user, nil)
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
	...
	var posts []*Post
	qs = dORM.QueryTable("post")
	num, err = qs.RelatedSel().All(&posts, nil)
	throwFail(t, err)
...
func TestReverseQuery(t *testing.T) {
	var profile Profile
	err := dORM.QueryTable("user_profile").Filter("User", 3).One(&profile, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))

	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "astaxie").One(&profile, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))

	var user User
	err = dORM.QueryTable("user").Filter("Posts__Title", "Examples").One(&user, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "astaxie"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "astaxie").Limit(1).One(&user, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "astaxie"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "astaxie").RelatedSel().Limit(1).One(&user, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "astaxie"))
	...
	var posts []*Post
	num, err := dORM.QueryTable("post").Filter("Tags__Tag__Name", "golang").All(&posts, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 3))
	throwFailNow(t, AssertIs(posts[0].Title, "Introduction"))

	posts = []*Post{}
	num, err = dORM.QueryTable("post").Filter("Tags__Tag__Name", "golang").Filter("User__UserName", "slene").All(&posts, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(posts[0].Title, "Introduction"))
	...
	num, err = dORM.QueryTable("post").Filter("Tags__Tag__Name", "golang").
		Filter("User__UserName", "slene").RelatedSel().All(&posts, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	...
	var tags []*Tag
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").All(&tags, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "astaxie").All(&tags, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "astaxie").RelatedSel().All(&tags, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
...
func TestPkManyRelated(t *testing.T) {
	...
	num, err := qs.Filter("Permissions__Permission", permission.ID).All(&groups, nil)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 2))
}
...
func TestInLineOneToOne(t *testing.T) {
	...
	err = dORM.QueryTable(il).Filter("Id", 1).RelatedSel().One(il, nil)
	...
	rinline := NewInLine()
	err = dORM.QueryTable(rinline).Filter("InLineOneToOne__Id", 1).One(rinline, nil)
